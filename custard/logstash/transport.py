...
class UdpTransport:
    ...
    def _close(self, force=False):
        if (not self._keep_connection or force) and self._sock:
            self._sock.close()
            self._sock = None
...
class TcpTransport(UdpTransport):
    ...
    def __init__(  # pylint: disable=too-many-arguments
        self, host, port, ssl_enable, ssl_verify, keyfile, certfile, ca_certs, timeout=TimeoutNotSet, **kwargs
    ):
...
class HttpTransport(Transport):
    ...
    is True the class tries to verify the TLS certificate with certify. If you
    ...
    def send(self, events: list, **kwargs):
        ...
        Max Events: `logstash.Constants.QUEUED_EVENTS_BATCH_SIZE`
