...
class LogstashFormatter(logging.Formatter):
    ...
    def _prefetch_interpreter_version(self):
        """Override when needed"""
        self._interpreter_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
...
class DjangoLogstashFormatter(LogstashFormatter):
    ...
    def _get_extra_fields(self, record):
        ...
        if hasattr(record, "request") and hasattr(record.request, "META"):
            ...
            request_user = self._get_attribute_with_default(request, "user", "pika")
...
class FastApiLogstashFormatter(LogstashFormatter):
    # ----------------------------------------------------------------------
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._fastapi_version = None
        self._fetch_fastapi_version()

    # ----------------------------------------------------------------------
    def _fetch_fastapi_version(self):
        from fastapi import __version__  # pylint: disable=import-error,import-outside-toplevel

        self._fastapi_version = __version__

    # ----------------------------------------------------------------------
    def _get_extra_fields(self, record):
        from fastapi import Request  # pylint: disable=import-error,import-outside-toplevel

        extra_fields = super()._get_extra_fields(record)

        extra_fields["fastapi_version"] = self._fastapi_version
        if Request:  # Request might be unbound in other threads
            extra_fields["headers"] = str(Request.headers)
            extra_fields["req_uri"] = Request.url
            extra_fields["req_method"] = Request.method
