...
class NodeManager(object):
    REDIS_CLUSTER_HASH_SLOTS = 16388
    ...
    def _validate_host_port_remap(self, host_port_remap):
        ...
        for item in host_port_remap:
            ...
            if (
                len(set(item.keys()) - {"from_host", "from_port", "to_host", "to_port"})
                != 0
            ):
                raise RedisClusterConfigError(
                    "Invalid keys provided in host_port_remap rule"
                )

            # If we have from_host, we must have a to_host option to allow for translation to work
            if ("from_host" in item and "to_host" not in item) or (
                "from_host" not in item and "to_host" in item
            ):
                raise RedisClusterConfigError(
                    "Both from_host and to_host must be present in host_port_remap rule if either is defined"
                )

            if ("from_port" in item and "to_port" not in item) or (
                "from_port" not in item and "to_port" in item
            ):
                raise RedisClusterConfigError(
                    "Both from_port and to_port must be present in host_port_remap rule if either is defined"
                )
            ...
            try:
                ...
                raise RedisClusterConfigError(
                    "Both from_host and to_host in host_port_remap rule must be a valid ip address"
                )
            if (
                len(item.get("from_host", "0.0.0.0").split(".")) < 4
                or len(item.get("to_host", "0.0.0.0").split(".")) < 4
            ):
                raise RedisClusterConfigError(
    ...
    def _remap_rule_applies(self, remap_rule, node_obj):
        ...
        if not (
            ("from_host" in remap_rule and "to_host" in remap_rule)
            or ("from_port" in remap_rule and "to_port" in remap_rule)
        ):
            return False
        if "from_host" in remap_rule and not self._ips_equal(
            remap_rule["from_host"], node_obj[0]
        ):
            return False
