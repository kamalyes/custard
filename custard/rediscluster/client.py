...
class RedisCluster(Redis):
    ...
    def from_url(
        ...
        **kwargs
        ...
        if url.lower().startswith("unix://"):
            raise RedisClusterException(
                "Unix sockets do not work in a cluster environment"
            )
    ...
    def cluster_failover(self, node_id, option=None):
        ...
        if option:
            if option.upper() not in ["FORCE", "TAKEOVER"]:
                raise RedisError(
                    "Invalid option for CLUSTER FAILOVER command: {0}".format(option)
                )
            else:
    ...
    def cluster_reset(self, node_id, soft=True):
        ...
        return self.execute_command(
            "CLUSTER RESET", b"SOFT" if soft else b"HARD", node_id=node_id
        )
    ...
    def msetnx(self, *args, **kwargs):
        ...
        # Iterate over all items and fail fast if one value is True.
