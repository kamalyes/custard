    ...
    def _check_size_and_clear(cls):
        ...
        def __check_size_and_clear():
            while 1:
                ...
                if size > 1000:
                    very_xprint(f'kafka防止意外日志积累太多了,达到 {size} 个，为防止内存泄漏，清除队列')
        ...
        while 1:
            try:
                if self._task_queue.qsize() > 10000:
                    very_xprint('防止意外日志积累太多了，不插入es了。')
...
class ColorHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord):
        ...
        try:
            ...
            very_xprint(e)
            very_xprint(traceback.format_exc())
...
class ConcurrentRotatingFileHandlerWithBufferInitiativeWin(ConcurrentRotatingFileHandler):
    ...
    """
    file_handler_list = []
    has_start_emit_all_file_handler = False  # 只能在Win运行正常，Win是多进程每个进程的变量has_start_emit_all_file_handler是独立的。linux是共享的。
    ...
    def _rollover_and_do_write(self):
        ...
        if buffer_msgs:
            try:
                ...
                # very_xprint(len(self._buffer_msgs))
                self.do_write(buffer_msgs)
            finally:
                self._do_unlock()


class ConcurrentRotatingFileHandlerWithBufferInitiativeLinux00000000(
    ...
    """
    file_handler_list = []
    has_start_emit_all_file_handler_process_id_set = set()  # 这个linux和Win都兼容，Win是多进程每个进程的变量has_start_emit_all_file_handler是独立的。linux是共享的。
    ...
    def rollover_and_do_write(self, ):
        # very_xprint(self.buffer_msgs_queue.qsize())
...
class CompatibleSMTPSSLHandler(handlers.SMTPHandler):
    ...
    def emit0(self, record: logging.LogRecord):
        ...
        if record.msg not in self._msg_map or time.time() - self._msg_map[record.msg] > self._time_interval:
            ...
            very_xprint(f' 邮件发送太频繁间隔不足60分钟，此次不发送这个邮件内容： {record.msg}    ')
...
class DingTalkHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        with self._lock:
            if time.time() - self._current_time > self._time_interval:
                # very_xprint(self._current_time)
                self._current_time = time.time()
                self.__emit(record)
                # Thread(target=self.__emit, args=(record,)).start()

            else:
                very_xprint(f' 此次离上次发送钉钉消息时间间隔不足 {self._time_interval} 秒，此次不发送这个钉钉内容： {record.msg}    ')
    ...
    def __emit(self, record):
        ...
        try:
            ...
            very_xprint(f'钉钉返回 ： {resp.text}')
        except requests.RequestException as e:
            very_xprint(f"发送消息给钉钉机器人失败 {e}")
...
class ConcurrentDayRotatingFileHandlerWin(logging.Handler):
    ...
    def _emit_all_file_handler(cls):
        while True:
            for hr in cls.file_handler_list:
                # very_xprint(hr.buffer_msgs_queue.qsize())
                hr._write_to_file()
