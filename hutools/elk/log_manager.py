...
import elk_config_default  # noqa
...
class MailHandlerConfig(DataClassBase):
    mailhost: tuple = elk_config_default.EMAIL_HOST
    fromaddr: str = elk_config_default.EMAIL_FROMADDR
    toaddrs: tuple = elk_config_default.EMAIL_TOADDRS
    subject: str = 'xx项目邮件日志报警'
    credentials: tuple = elk_config_default.EMAIL_CREDENTIALS
    secure = None
    timeout = 5.0
    is_use_ssl = True
...
class LogManager(object):
    ...
    def __init__(self, logger_name: typing.Union[str, None] = 'elk_default_namespace'):
        ...
        if logger_name in (None, '', 'root') and multiprocessing.process.current_process().name == 'MainProcess':
            very_xprint('logger_name 设置为None和root和空字符串都是一个意义，在操作根日志命名空间，任何其他日志的行为将会发生变化，'
        ...
        self._is_add_elastic_handler = None
        self._is_add_kafka_handler = None
        self._ding_talk_token = None
        self._ding_talk_time_interval = None
        self._mail_handler_config = None
        self._is_add_mail_handler = None
    ...
    def get_logger_and_add_handlers(self, log_level_int: int = None, *, is_add_stream_handler=True,
       ...
       """
        if log_level_int is None:
            log_level_int = elk_config_default.LOG_LEVEL_FILTER
        if do_not_use_color_handler is None:
            do_not_use_color_handler = not elk_config_default.DEFAULT_USE_COLOR_HANDLER
        if log_filename is None and elk_config_default.DEFAULT_ADD_MULTIPROCESSING_SAFE_RATING_FILE_HANDLER:
            log_filename = f'{self._logger_name}.log'
        if log_file_size is None:
            log_file_size = elk_config_default.LOG_FILE_SIZE
        if log_path is None:
            # print(elk_config_default.LOG_PATH)
            log_path = elk_config_default.LOG_PATH or '/pythonlogs'
        if formatter_template is None:
            formatter_template = elk_config_default.FORMATTER_KIND
        ...
        self._log_file_handler_type = log_file_handler_type or elk_config_default.LOG_FILE_HANDLER_TYPE
        ...
        if isinstance(formatter_template, int):
            self._formatter = elk_config_default.FORMATTER_DICT[formatter_template]
    ...
    def look_over_all_handlers(self):
        very_xprint(f'{self._logger_name}名字的日志的所有handlers是--> {self.logger.handlers}')
    ...
    def __add_handlers(self):
        ...
        if not (self._judge_logger_has_handler_type(ConcurrentRotatingFileHandler) or
                self._judge_logger_has_handler_type(ConcurrentRotatingFileHandlerWithBufferInitiativeWin) or
            ...
            if self._log_file_handler_type == 1:
                if os_name == 'nt':
                    ...
                    file_handler = ConcurrentRotatingFileHandlerWithBufferInitiativeWin(log_file,
                                                                                        maxBytes=self._log_file_size * 1024 * 1024,
                                                                                        backupCount=elk_config_default.LOG_FILE_BACKUP_COUNT,
                                                                                        encoding="utf-8")
                elif os_name == 'posix':
                ...
                file_handler = ConcurrentDayRotatingFileHandler(self._log_filename, self._log_path,
                                                                back_count=elk_config_default.LOG_FILE_BACKUP_COUNT)
        ...
        if not self._judge_logger_has_handler_type(
                ElasticHandler) and self._is_add_elastic_handler and elk_config_default.RUN_ENV == 'test':  # 使用kafka。不直接es。
            """
            生产环境使用阿里云 oss日志，不使用这个。
            """
            handler = ElasticHandler([elk_config_default.ELASTIC_HOST], elk_config_default.ELASTIC_PORT)
            handler.setLevel(self._logger_level2)
            self.__add_a_hanlder(handler)
        ...
        if not self._judge_logger_has_handler_type(
                KafkaHandler) and elk_config_default.RUN_ENV == 'test' \
                and elk_config_default.ALWAYS_ADD_KAFKA_HANDLER_IN_TEST_ENVIRONENT:
            handler = KafkaHandler(elk_config_default.KAFKA_BOOTSTRAP_SERVERS, )
            handler.setLevel(self._logger_level2)
            self.__add_a_hanlder(handler)
