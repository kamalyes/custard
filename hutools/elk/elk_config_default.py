"""
此文件elk_config.py是自动生成到python项目的根目录的
在这里面写的变量会覆盖此文件elk_config_default中的值。对elk包进行默认的配置
但最终配置方式是由get_logger_and_add_handlers方法的各种传参决定，如果方法相应的传参为None则使用这里面的配置
如果反对日志有各种彩色，可以设置 DEFAULT_USE_COLOR_HANDLER = False
如果反对日志有块状背景彩色，可以设置 DISPLAY_BACKGROUND_COLOR_IN_CONSOLE = False
如果想屏蔽elk包对怎么设置pycharm的颜色的提示，可以设置 WARNING_PYCHARM_COLOR_SETINGS = False
...
DEFAULT_USE_COLOR_HANDLER = True  # 是否默认使用有彩的日志。
DISPLAY_BACKGROUND_COLOR_IN_CONSOLE = True  # 在控制台是否显示彩色块状的日志。为False则不使用大块的背景颜色。
AUTO_PATCH_PRINT = True  # 是否自动打print的猴子补丁，如果打了猴子补丁，print自动变色和可点击跳转。
SHOW_PYCHARM_COLOR_SETTINGS = True  # 有的人很反感启动代码时候提示教你怎么优化pycharm控制台颜色，可以把这里设置为False

DEFAULT_ADD_MULTIPROCESSING_SAFE_RATING_FILE_HANDLER = False  # 是否默认同时将日志记录到记log文件记事本中。
...
FORMATTER_DICT = {
    ...
    2: logging.Formatter(
        ...
        "%Y-%m-%d %H:%M:%S"),
    3: logging.Formatter(
        '%(asctime)s - %(name)s - 【 File "%(pathname)s", line %(lineno)d, in %(funcName)s 】 '
    ...
    5: logging.Formatter(
        '%(asctime)s - %(name)s - "%(pathname)s:%(lineno)d" - %(funcName)s '
        '- %(levelname)s - %(message)s %Y-%m-%d %H:%M:%S'),  # 我认为的最好的模板,推荐
    ...
    9: logging.Formatter(
        '[p%(process)d_t%(thread)d] %(asctime)s - %(name)s - "%(pathname)s:%(lineno)d" '
    ...
    11: logging.Formatter(
        f'({computer_ip},{computer_name})-[p%(process)d_t%(thread)d] %(asctime)s - %(name)s '
        f'- "%(filename)s:%(lineno)d" - %(levelname)s - %(message)s',
